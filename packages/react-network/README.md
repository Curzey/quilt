...
**Note:** `useAcceptLanguage` calls `useRequestHeader`, so the rules on client-side renders applies here too. Wrap your app in a `NetworkUniversalProvider` and pass in `['Accept-Language']` to the `headers` prop in order to call `useAcceptLanguage` on subsequent client-side renders.
